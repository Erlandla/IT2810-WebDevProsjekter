...
describe("Renders, searches, filters and sorts movie titles", () => {
  ...
  it("Appends movies to Watchlist", () => {
    ...
    cy.get(".FilmInstance_title__CAfOh").click();
    cy.get(".Modal_btnAddToWatchlist__zcaOR").click();
    cy.get('[data-cy="watchListButton"]').click();
    cy.contains("The Lucky Horseshoe").should("be.visible");
    cy.get(".FilmInstance_title__CAfOh").click();
    cy.get(".Modal_btnRemoveFromWatchlist__Zk\\+RG").click();
  ...
  it("Changes Movie Ratings", () => {
    ...
    cy.contains("The Lucky Horseshoe").should("be.visible");
    cy.get(".FilmInstance_rating__WJi3T")
      .first()
      .invoke("text")
      .as("movieRating");
    ...
    cy.get(
      '[style="display: flex; flex-flow: row wrap; margin-bottom: 1vh;"] > :nth-child(8)'
    ).click();
  ...
  // We check that movies should 'not exist' as only 10 movies are rendered at a time as to not overload the computer
  ...
  it("Filters adult movies", () => {
    ...
    cy.get('[data-cy="ADULT MOVIES: Disable"]').click();
  ...
  it("Sorts movies", () => {
    ...
    cy.contains("El compromiso").should("be.visible");
    cy.contains("The Shawshank Redemption").should("not.exist");
    //sorts by Rating
    cy.get('[data-cy="ratingSorting"]').click();
    cy.contains("Three Moons of Biyangdo").should("be.visible");
    cy.contains("Stand Up the Face Fear").should("not.exist");
    cy.get('[data-cy="ratingSorting"]').click();
    cy.contains("Stand Up Face the Fear").should("be.visible");
    cy.contains("Three Moons of Biyangdo").should("not.exist");

    //sorts by release'[data-cy="submit"]'
